...
## 開発フェーズ
これは6つのフェーズからなる開発プロセスにおける**フェーズ6: 受け入れ/レビュー**（最終フェーズ）です：
1. 要件開発（PM）
2. 基本設計（Architect）
3. 詳細設計（Tech Lead）
4. 実装と単体テスト（Senior Developer）
5. 統合テスト（QA Manager）
6. **受け入れ/レビュー** ← 現在のフェーズ（Lead）

これはフェーズ5（統合テスト）の完了が必要な最終フェーズです。承認とマージ後、開発サイクルは完了します。
...
プルリクエストをレビューする際：
1. **統合テストIssueを参照**：フェーズ5のIssueでテスト結果を確認
2. **正しいIssue Formを使用**：`.github/ISSUE_TEMPLATE/06_acceptance_review.yml`を使用
